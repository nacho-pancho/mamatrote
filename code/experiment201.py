...
def model_vs_scale_and_npoints(m,n,
    ...
    affine_set_1 = sim_affine_model(n, m, model_dist, rng)
    nscales = len(scales)
    nnp   = len(npointses)
    ...
    for i,npoints in enumerate(npointses):
        ...
        for k in range(nsamp):
            model_points = sim_patch_points(nmodel, affine_set_1, scatter, rng)
            back_points  = bg_dist((nback, n))
            _test_points = np.concatenate((model_points,back_points))
            for j,s in enumerate(scales):
                nfa = nfa_ks(_test_points, affine_set_1, m, m+1, distance_to_patch, s)
                nfas[i,j] += nfa < 1
    return  nfas/nsamp
