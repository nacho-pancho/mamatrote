...
def model_vs_scale_and_npoints(m,n,
    ...
    affine_model = sim_affine_model(m, bounding_box, rng)
    nscales = len(scales)
    nnp   = len(npointses)
    ...
    for i,npoints in enumerate(npointses):
        ...
        for k in range(nsamp):
            model_points = sim_affine_points(affine_model, nmodel, bounding_box, scatter, rng, scatter_dist)
            back_points  = sim_points(nback, bounding_box, rng)
            _test_points = np.concatenate((model_points,back_points))
            for j,s in enumerate(scales):
                nfa = nfa_ks(_test_points, affine_model, s)
                nfas[i,j] += nfa < 1
    return  nfas/nsamp
