...
def oblique_vs_angle(m, n,
    ...
    # we rotate about the z axis, so we need an patch set that is orthogonal to the z axis
    c = np.zeros(n)
    I = np.eye(n)
    V = I[:m,:]
    W = I[m:,:]
...
if __name__ == "__main__":
    ...
    for n in (2,3):
        for m in range(1,n):
            print(f"n={n} m={m}")
            fbase  = (f'patch NFA vs scale and angle n={n} m={m} s={scatter} N={npoints}').lower().replace(' ','_').replace('=','_')
