#!/usr/bin/env python3
"""
This experiment investigates the ability to detect patch structures
as a function of:
...
def model_vs_scale_and_distro(m,n,
    ...
    patch_set = sim_patch_set(n,m,model_dist,rng)
    ...
    for i,scatdist in enumerate(scatter_distros):
        ...
        for k in range(nsamp):
            ...
            for j,s in enumerate(scales):
                nfa = nfa_ks(_test_points, patch_set, m, m+1, distance_to_patch, s)
                nfas[i,j] += nfa < 1
    return nfas*(1/nsamp)
...
if __name__ == "__main__":
    ...
    for n in (2,3):
        for m in range(n):
            ...
            fbase  = (f'patch NFA vs scale and decay factor n={n} m={m} s={scatter} N={npoints}').lower().replace(' ','_').replace('=','_')
