#!/usr/bin/env python3
"""
This experiment investigates the ability to detect sphere structures
as a function of:
...
def model_vs_scale_and_distro(m,n,
    ...
    for i,scatdist in enumerate(scatter_distros):
        ...
        for k in range(nsamp):
            model_points = sim_sphere_cloud(sphere_set, nmodel, rng, scatter, model_dist, scatter_distro=scatdist)
            back_points = bg_dist((nback, n))
            _test_points = np.concatenate((model_points,back_points))
            for j,s in enumerate(scales):
                nfa = nfa_ks(_test_points, sphere_set, m, m+1, distance_to_sphere, s)
                nfas[i,j] += nfa < 1
    return nfas*(1/nsamp)
...
if __name__ == "__main__":
    ...
    for n in (2,3):
        for m in range(n):
            ...
            fbase  = (f'sphere NFA vs scale and decay factor n={n} m={m} s={scatter} N={npoints}').lower().replace(' ','_').replace('=','_')
